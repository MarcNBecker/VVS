...
function Blocklage() {
	this.kursID = 0;
	this.semester = 0;
	this.startDatum = ""; //yyyy-MM-dd
	this.endDatum = ""; //yyyy-MM-dd
	this.raum = "";
}
...
Dozent.prototype.StatusEnum = {
		1: "Neu",
		2: "Aktiv",
		3: "Inaktiv"
};

Dozent.prototype.GeschlechtEnum = {
		0: "M",
		1: "F",
};
...
var webService = new function() {
	...
	this.getKursBlocklagen = function(k, c) {
	...
	this.createFachInstanz = function(mi, fi, c) {
		self.doRequest(fachInstanzenURI.replace("{modulplanID}", mi.modulplanID).replace("{modulID}", mi.modul.id).replace("{fachID}", fi.fach.id), "PUT", fi, c);	
	};
	
	this.createFachInstanz = function(mi, fi, c) {
		self.doRequest(fachInstanzenURI.replace("{modulplanID}", mi.modulplanID).replace("{modulID}", mi.modul.id).replace("{fachID}", fi.fach.id), "DELETE", null, c);	
	};
